...
func TestCreateBookingReturnsNotFoundWhenSlotDoesNotExist(t *testing.T) {
	databaseURL := testutil.PrepareDatabase(t)
	...
	token := "booking-token"
	clientID := "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
	insertClientSession(t, ctx, db, clientID, "+79990001004", token)
	router := bookingRouter(db)

	recorder := performCreateBooking(
		router,
		token,
		"77777777-7777-7777-7777-777777777778",
		`{"slot_id":"00000000-0000-0000-0000-000000000001","seats_count":1,"rental_count":0}`,
	)
	...
	if response.Code != httpapi.CodeNotFound {
		t.Fatalf("code = %q, want %q", response.Code, httpapi.CodeNotFound)
	}
}
